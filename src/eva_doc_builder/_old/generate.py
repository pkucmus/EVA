...
def render_assembly_page(name, assembly, context):
    ...
    def add_parts(parts, parts_collection):
    ...
    for sub_assembly in assembly["sub_assemblies"]:
        if isinstance(sub_assembly, dict):
            for subs in sub_assembly.values():
                for sub in subs:
                    parts = add_parts(parts, data["sub_assemblies"][sub]["parts"])
        else:
            parts = add_parts(parts, data["sub_assemblies"][sub_assembly]["parts"])
